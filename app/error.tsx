...
export default function ErrorPage({ error, reset }: { error: Error; reset: () => void }) {
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, flexDirection: "column", textAlign: "center" }}>
      ...
      <p style={{ color: "var(--text-muted)", marginTop: 6, maxWidth: 360 }}>{error.message || "Algo apagou o fogo. Tenta de novo."}</p>
      <div style={{ display: "flex", gap: 10, marginTop: 22 }}>
        ...
        <Link href="/" style={{ padding: "12px 22px", borderRadius: 12, background: "var(--surface)", border: "1px solid rgba(255,255,255,0.08)", color: "var(--text)", textDecoration: "none" }}>
          Início
        </Link>
      </div>
    </div>
